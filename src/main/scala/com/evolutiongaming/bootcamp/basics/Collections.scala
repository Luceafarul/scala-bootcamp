...
object MutabilityAndRt extends App {
...
object Sets extends App {
  ...
  println(beautifulNumber.contains(3)) // apply(3) the same as SetName.contains(3)
...
object LinkedList extends App {
  ...
  def findGap(l: List[Int]): Option[(Int, Int)] = {
    l match {
      case Nil => None
      case _ :: tail => l.zip(tail).find { case (first, second) => second - first > 1 }
      //      case _ :: Nil => None
  ...
  def min(list: List[Int]): Option[Int] = {
    //    @tailrec
    //    def loop(list: List[Int], min: Option[Int]): Option[Int] = {
    //      list match {
    //        case Nil => min
    //        case head :: tail if (min.isEmpty) => loop(tail, Some(head))
    //        case head :: tail => if (head < min.get) loop(tail, Some(head)) else loop(tail, min)
    //      }
    //    }
    //
    //    loop(list, Option.empty[Int])

    list.foldRight(Option.empty[Int]) {
      case (a, b) => b.fold(Some(a))(elem => Some(a min elem))
    }
  }
  ...
  def scanLeft[T](zero: T)(list: List[T])(f: (T, T) => T): List[T] = {
    @tailrec
    def loop(xs: List[T], res: List[T]): List[T] = {
      xs match {
        case Nil => res
        case head :: tail => loop(tail, res :+ f(res.last, head))
      }
    }
  ...
  def count(s: String): List[(Char, Int)] = {
    s.foldLeft(List.empty[(Char, Int)]) {
      case (acc, elem) =>
        if (acc.isEmpty || acc.last._1 != elem) acc :+ (elem -> 1)
        else acc.init :+ (elem -> (acc.last._2 + 1))
    }
  }
